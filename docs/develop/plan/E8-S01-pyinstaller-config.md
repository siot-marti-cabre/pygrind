...
## Tasks
- **T1: Create spec file** -- Write pygrind.spec: Analysis pointing to src/pygrind/__main__.py, datas=[('exercises/', 'exercises/')], hiddenimports=['PyQt6', 'QScintilla', 'yaml']. Use --onedir mode.
- **T2: Create build script** -- scripts/build.py or Makefile target running pyinstaller pygrind.spec. Document prerequisites (PyInstaller installed in venv).
...
- `src/pygrind/paths.py` — Runtime path resolution for frozen/dev environments (~20 lines)
- `pygrind.spec` — PyInstaller spec with hidden imports, exercises data, onedir mode (~65 lines)
...
**Key Decisions:**
- Created `pygrind.paths` module for `sys._MEIPASS` resolution — reusable by loader
